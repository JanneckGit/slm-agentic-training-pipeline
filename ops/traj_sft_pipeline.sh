...
# Sequential (GB10 has no MIG). Eval = rollout.py on the 40 heldout_eval tasks through the same verifier
...
serve() {  # $1=model $2=served-name-model $3=gpu_util
  ...
  VLLM_MODEL="$1" VLLM_GPU_UTIL="${3:-0.85}" VLLM_MAX_MODEL_LEN="8192" \
...
eval_heldout() {  # $1=served-model $2=label $3=outfile
  timeout 1800 env PYTHONPATH="$REPO" LOGURU_LEVEL=ERROR "$TAU2PY" \
    ...
    --max-turns 8 --max-tokens-per-turn 2048 --max-regen 1 --concurrency 4 \
    --output "$3" 2>&1 | tail -6
}
